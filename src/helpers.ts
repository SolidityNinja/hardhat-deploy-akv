    ...
    if (from.length >= 64) {
      ...
      if (availableAccounts[from.toLowerCase()]) {
        ...
        if (registeredProtocol) {
          if (registeredProtocol === 'external') {
          ...
          } else if (registeredProtocol.startsWith('azure')) {
            const protocolSplit = registeredProtocol.split('://');
            ...
            ethersSigner = new AzureKeyVaultSigner(keyVaultCredentials, provider);
