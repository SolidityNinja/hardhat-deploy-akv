...
import {AzureKeyVaultSigner} from 'ethersjs-akv-signer-extension'
...
async function transformNamedAccounts(
  ...
  if (configNamedAccounts) {
    ...
    async function parseSpec(spec: any): Promise<string | undefined> {
        ...
        case 'string':
          ...
          if (protocolSplit.length > 1) {
            if (protocolSplit[0].toLowerCase() === 'external') {
              ...
              addressesToProtocol[address.toLowerCase()] =
                protocolSplit[0].toLowerCase();
            } else if (protocolSplit[0].toLowerCase() === 'azure') {
              const keyVaultInfoSplit = protocolSplit[1].split('/');
              const keyVaultCredentials = {
                keyName: keyVaultInfoSplit[1],
                vaultUrl: "https://" + keyVaultInfoSplit[0],
              };
              let azureKeyVaultSigner = new AzureKeyVaultSigner(keyVaultCredentials);
              address = await azureKeyVaultSigner.getAddress();
            ...
            if (spec.slice(0, 2).toLowerCase() === '0x') {
              ...
              address = await parseSpec(configNamedAccounts[spec]);
            }
          }
          break;
        case 'number':
          ...
          if (spec) {
            if (spec.type === 'object') {
              ...
              if (typeof newSpec !== 'undefined') {
                address = await parseSpec(newSpec);
              }
            }
          }
          break;
    ...
    for (const accountName of accountNames) {
      ...
      const address = await parseSpec(spec);
      if (address) {
...
export async function processNamedAccounts(
  network: Network,
  namedAccounts: {
    [name: string]:
      | string
  ...
  if (namedAccounts) {
    return await transformNamedAccounts(
      namedAccounts,
      chainIdGiven,
      accounts,
